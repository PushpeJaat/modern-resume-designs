...
const s = StyleSheet.create({
  ...
  contactPill: { flexDirection: "row", alignItems: "center", paddingHorizontal: 10, paddingVertical: 5, borderRadius: 12, borderWidth: 1, borderColor: "#ddd5f5", backgroundColor: "#faf8ff", fontSize: 9 },
  contactIcon: { color: colors.primary, fontWeight: "bold", marginRight: 5, fontSize: 8 },
  summaryBox: { padding: 14, backgroundColor: "#f5f3ff", borderRadius: 8, borderWidth: 1, borderColor: "#ede9fe", marginBottom: 18 },
  ...
  skillBox: { padding: 10, backgroundColor: "#faf8ff", borderRadius: 6, borderWidth: 1, borderColor: "#ede9fe", marginBottom: 10 },
  ...
  expCard: { padding: 12, backgroundColor: "#faf8ff", borderRadius: 6, borderLeftWidth: 3, marginBottom: 10 },
  ...
  expDate: { fontSize: 8, color: colors.gray, backgroundColor: "#ede9fe", paddingHorizontal: 8, paddingVertical: 3, borderRadius: 10 },
    ...
    <Document>
      <Page size="A4" style={s.page} wrap>
        ...
        <View style={s.contactRow} wrap={false}>
          {d.personalInfo.email && <View style={s.contactPill}><Text style={s.contactIcon}>✉</Text><Text>{d.personalInfo.email}</Text></View>}
          {d.personalInfo.phone && <View style={s.contactPill}><Text style={s.contactIcon}>☎</Text><Text>{d.personalInfo.phone}</Text></View>}
          {d.personalInfo.location && <View style={s.contactPill}><Text style={s.contactIcon}>📍</Text><Text>{d.personalInfo.location}</Text></View>}
        </View>
        ...
        <View style={s.body}>
                  ...
                  <View key={i} style={s.skillBox}>
                    ...
                    <View style={{ flexDirection: "row", flexWrap: "wrap" }}>
                      {cat.skills.filter(sk => sk.trim()).map((skill, idx) => (
                        <Text key={idx} style={{ ...s.skillTag, backgroundColor: i === 0 ? colors.primary : i === 1 ? colors.accent : colors.dark, color: colors.white }}>{skill}</Text>
                      ))}
                    </View>
                  </View>
                ))}
          ...
          <View style={s.rightCol}>
            {d.experience.length > 0 && d.experience[0].position && (
              <View style={s.section}>
                <Text style={{ ...s.sectionTitle, fontSize: 15 }}>Experience</Text>
